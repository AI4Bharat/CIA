...
import os
os.environ['HF_HOME'] = '/data/safi/cia/cache'
...
def get_predictions(n_gpus, model, type, lang):
    # model_path = os.path.join(BASE_MODEL_PATH, model_name)
    tokenizer = AutoTokenizer.from_pretrained(model)

    tokenizer.chat_template = get_chat_template(type)
    
    
    print(f"Loading dataset for {lang}")
    # dataset = load_dataset("ai4bharat/cia-bench", lang)
    dataset = load_dataset('json', data_files=f"/data/safi/cia/cia-bench/data/{lang_map[lang]}_testset.jsonl")
    prompts = [tokenizer.apply_chat_template([row['messages'][0]], tokenize=False, add_generation_prompt=True) for row in dataset['train']]
    ground_truth_scores = [row['orig_score'] for row in dataset['train']]
    ground_truth_feedbacks = [row['orig_feedback'] for row in dataset['train']]
...
def parse_args():
    ...
    parser.add_argument("--type", type=str, required=True, help="type of model", choices=["llama", "gemma"])
    parser.add_argument("--output_path", type=str, default="/data/safi/cia/CIA/artifacts")
...
def main(args):
    ...
    os.makedirs(f"{args.output_path}/results/{args.lang}", exist_ok=True)
    ...
    os.makedirs(f"{args.output_path}/predictions/{args.lang}", exist_ok=True)
